...
int cnum;
    ...
    for (int i = 0; i < voter_count; i++)
    {
        cnum = preferences[0][i];
        candidates[cnum].votes++;
    }
    printf("%s: %i\n%s: %i\n%s: %i\n", candidates[0].name, candidates[0].votes, candidates[1].name, candidates[1].votes, candidates[2].name, candidates[2].votes);
    return;
}
